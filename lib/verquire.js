...
module.constructor.prototype.require = function(name) {
    ...
    if (collector && !require_reported[name]) {
        require_reported[name] = 1;
        collector(name);
    }
    return result;
};
...
function create_collector() {
    ...
    return function ga(name, e) {
        var event = {
            ...
            ea: name,
            ec: e ? 'FAILED_REQUIRE' : 'REQUIRE'
        };

        if (e)
            event.el = (e.message || e.toString()).substring(0, 255);

        // Form-url-encode the event data.

        var data = url.format({ query: event }).substring(1);

        // Best effort send the Google Analytics request, do not wait for response.
